...
message("- **COP15** a renvoyé `NaN` pour le t-test et la taille d’effet (identiques, moyenne=4.67, ET=0.52).")
...
################################################################################
# ──────────────────────────────────────────────────────────────────────────────
# 1. Passage en format long (si besoin) & harmonisation des noms
# ──────────────────────────────────────────────────────────────────────────────
if (!exists("data_long")) {
  pre  <- pre  %>% mutate(Participant = row_number())
  post <- post %>% mutate(Participant = row_number())

  # valeurs_to = "Valeur" → on renomme plus bas
  data_long <- bind_rows(
...
# Renomme seulement si la colonne existe (plus d’arrêt inopiné)
if ("Valeur" %in% names(data_long)) {
  data_long <- data_long %>% rename(Value = Valeur)
}

# ──────────────────────────────────────────────────────────────────────────────
# 2. Résumé statistique : moyennes, SD, p-value, étoiles
# ──────────────────────────────────────────────────────────────────────────────
...
summary_cop <- data_long %>%
  ...
  get_summary() %>%
  mutate(num = as.integer(str_extract(Variable, "\\d+"))) %>%
  arrange(num) %>%
  mutate(Variable = factor(Variable,
                           levels = paste0("COP", sort(unique(num))))) %>%
  select(-num)

# ──────────────────────────────────────────────────────────────────────────────
# 3. Fonction de tracé améliorée
# ──────────────────────────────────────────────────────────────────────────────
plot_domain <- function(df_summary, titre,
                        y_limits,
                        col_pre = "steelblue", col_post = "firebrick") {

  ## (a) Plus d’écart horizontal entre Pré et Post
  pd <- position_dodge(width = 0.6)

  ## (b) Coordonnées des annotations
  annot <- df_summary %>%
    group_by(Variable) %>%
    summarise(
      y_text = y_limits[2] - diff(y_limits) * 0.04,  # texte
      y_line = y_limits[2] - diff(y_limits) * 0.08,  # crochet
      sig    = first(sig),
      .groups = "drop"
    )
  ...
  ## (c) Data « fantôme » pour la légende des p-values
  legend_df <- tibble(
  ...
  ggplot(df_summary,
    ...
    guides(
      colour = guide_legend(order = 1, label.hjust = 0),
    ...
    scale_y_continuous(
      limits = y_limits,
      breaks = seq(y_limits[1], y_limits[2], 1),                    # entiers
      labels = scales::number_format(accuracy = 1),
      expand = expansion(mult = c(0, 0.12))                         # marge haut
    ) +
    labs(title = titre,
         y     = "Moyenne ± ET",
         x     = NULL) +
    ...
    theme(
      axis.text.x       = element_text(angle = 45, hjust = 1,
                                       size = 14, face = "bold"),
      legend.box        = "vertical",
      legend.box.just   = "left",  # bloc aligné gauche
      legend.text.align = 0,
      legend.title.align= 0
    ) +
    coord_cartesian(clip = "off")  # garde les crochets même hors panneau
}

# ──────────────────────────────────────────────────────────────────────────────
# 4. Création des figures et export PNG
# ──────────────────────────────────────────────────────────────────────────────
